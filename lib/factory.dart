...
class MaFactory {
  static MaFactory _singleton;

  Map<String, Logger> _loggers = {};
  Store<AppState> _store;
  PersistCookieJar _cookieJar;
  MaService _wgService;

  MaFactory._();

  factory MaFactory() {
    if (_singleton == null) {
      _singleton = MaFactory._();
    }
    return _singleton;
  }
  ...
  Logger getLogger(String name) {
    if (_loggers[name] == null) {
      Logger.root.level = MaConfig.loggerLevel;
  ...
  Future<Store<AppState>> getStore() async {
    if (_store == null) {
      final persistor = Persistor<AppState>(
        storage: FlutterStorage(key: MaConfig.packageInfo.packageName),
        ...
        transforms: Transforms(
          onLoad: [
                (state) {
              if (compareVersion(
                  state.version, MaConfig.packageInfo.version, 2) !=
                  0) {
                state = AppState();
              }
              return state;
  ...
  Future<MaService> getMaService() async {
    if (_wgService == null) {
      var cookieJar = await getCookieJar();
      _wgService = MaService(cookieJar);
    }
    return _wgService;
  }
}